...
#define IXGBE_PRIV_FLAGS_VLAN_STAG_FILTER		BIT(4)
	"vlan-stag-filter",
#define IXGBE_PRIV_FLAGS_OUTER_VLAN_PROTO_8021AD	BIT(5)
			   ...
			   void *p)
{
	...
	case ixgbe_mac_X550EM_a:
		regs_buff[1145] = IXGBE_R32_Q(hw, IXGBE_DMATXCTL);
		regs_buff[1146] = IXGBE_R32_Q(hw, IXGBE_EXVET);
		break;
	default:
		break;
	}
...
static int ixgbe_set_priv_flags(struct net_device *netdev, u32 priv_flags)
{
	...
	if (changed & IXGBE_PRIV_FLAGS_AUTO_DISABLE_VF) {
